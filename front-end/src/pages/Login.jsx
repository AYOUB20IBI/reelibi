...
export default function Login() {
  ...
  const LoginSubmit = async (e)=>{
    e.preventDefault()
    await LoginApi(dataForm?.email,dataForm?.password).then(res=>{
      if(res?.status===200){
      ...
      if(err.response.status === 422){
        ...
        setErrors({
          ErrorAll: err.response.data.message ? err.response.data.message : ''
        })
      }
    })
  }
                  ...
                  <div className="row gy-2 overflow-hidden">
                    <div className="col-12">
                      <div className="form-floating mb-2">
                        <input type="email" className="form-control shadow-none" name="email" id="email" placeholder="name@example.com" 
                    ...
                    <div className="col-12">
                      <div className="form-floating mb-2">
                        <input type={showPass? 'text':'password'} className="form-control shadow-none" name="password" id="password"  placeholder="Password" 
                    ...
                    <div className="col-12">
                      <div className="form-check">
                        <input className="form-check-input" type="checkbox" name="remember_me" id="remember_me" onChange={(e)=>setShowPass(e.target.checked)}/>
                    ...
                    <div className="col-12">
                      <div className="d-grid">
                        <button className="btn btn-dark btn-lg" type="submit" onClick={LoginSubmit}>Login</button>
                      </div>
                    </div>
                    <div className="col-12">
                        <p className='text-center' id={loginStyle.forgot_password}><Link className='text-decoration-none' to={"/forgot/password"}>Forgot Password ?</Link></p>
                    </div>
                  </div>
                </form>
              </div>
